...
def comments_for_tea(tea_id: int, db: Session = Depends(get_db)):
...
def single_comment(comment_id: int, db: Session = Depends(get_db)):
    comment = db.query(CommentModel).filter(CommentModel.id == comment_id).first()
    
    if not comment:
        raise HTTPException(status_code=404, detail="Error Data Entry")
...
def add_comments(comment: CommentSchema, db: Session = Depends(get_db)):
